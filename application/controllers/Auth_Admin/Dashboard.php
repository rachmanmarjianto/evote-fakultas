...
class Dashboard extends CI_Controller {
    ...
    public function index()
	{
        ...
        $data['hadir'] = $this->db->query("SELECT COUNT(pemilih_id) AS hadir FROM tb_pemilih WHERE pemilih_id in (select pemilih_id from tb_suara)")->row_array();
        $data['tdkhadir'] = $this->db->query("SELECT COUNT(pemilih_id) AS tdkhadir FROM tb_pemilih WHERE pemilih_id not in (select pemilih_id from tb_suara)")->row_array();
